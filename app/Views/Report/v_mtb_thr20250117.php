...
<script>
    ...
    $('#printToFile').on("click", function(){
            ...
            var diffMonths = (endDates.getFullYear() - startDates.getFullYear()) * 12;
            diffMonths -= startDates.getMonth();
            diffMonths += endDates.getMonth();

            var numMonths = diffMonths <= 0 ? 0 : diffMonths;
            if (numMonths < 12 || numMonths > 12) {
              $.notify({
                title: "Erorr : ",
                message: "Jarak Antar Bulan Harus 12 Bulan",
                icon: 'fa fa-times' 
              },{
                type: "danger",
                delay: 1000
