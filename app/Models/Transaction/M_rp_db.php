...
class M_rp_db extends Model
{
     ...
     public function getStartDateSM($month, $year, $SM)
     {
          $stQuery = "SELECT start_date FROM tr_timesheet WHERE month_process = '$month' AND year_process = '$year' AND payroll_group = '$SM' LIMIT 1";
          $query  = $this->db->query($stQuery); 
          $arrayRow = $query->getRowArray(); 
          return $arrayRow;
     }
     ...
     {
          $stQuery  = "SELECT a.*,SUM(work_day) AS total_wd, MIN(date_process) AS mindate, MAX(date_process) AS maxdate ,b.full_name,b.dept,b.emp_position AS position FROM rp_db a, mt_biodata_01 b  WHERE date_process BETWEEN '".$this->startDate."' AND '".$this->endDate."' AND a.biodata_id = b.biodata_id /*AND a.slip_id='240200054'*/  group BY a.biodata_id ;";   
          // echo $stQuery;
          // exit();
          $query = $this->db->query($stQuery)->getResultArray();
          return $query;
     ...
     public function getDataDbByPerDate($id,$date)
     {
          $stQuery  = "SELECT * FROM ".$this->table." WHERE date_process = '".$date."' AND biodata_id = '".$id."' ORDER BY date_process;";
          // echo $stQuery;
          // exit();
          $query = $this->db->query($stQuery)->getRowArray();
          // dd($query['in_db']);
