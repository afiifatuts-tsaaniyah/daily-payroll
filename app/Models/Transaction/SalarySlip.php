...
";
    ...
    public function getEndWork($biodataId, $yearPeriod, $monthPeriod)
    {
        ...
        // 1️⃣ Cari bulan END terdekat (>= monthPeriod)
        $sqlEndTerdekat = "
    ...
    ";
        ...
        // 3️⃣ Bulan pertama dalam periode
        $sqlBulanPertama = "
        SELECT month FROM tr_tax
        WHERE year = ?
        AND biodata_id = ?
    ...
    ";
        ...
        // 4️⃣ Cari bulan END terakhir (> monthPeriod)
        $sqlEndTerakhir = "
        SELECT month FROM tr_tax
        WHERE year = ?
        AND month > ?
    ...
    ";
        ...
        // 5️⃣ Hitung jumlah bulan dalam periode
    ...
    public function getStartWork($biodataId, $yearPeriod, $monthPeriod)
    {
        ...
        // 1️⃣ Query bulan pertama yang ada datanya
        $sqlBulanPertama = "
        SELECT month FROM tr_tax
        WHERE biodata_id = ?
        AND year = ?
    ...
    ";
        ...
        // 2️⃣ Cari bulan END terdekat sebelum monthPeriod
        $sqlBulanEND = "
        SELECT month FROM tr_tax
        WHERE year = ?
        AND month < ?
    ...
    ";
        ...
        // 3️⃣ Jika tidak ada END sama sekali → kembalikan bulan pertama
        if (is_null($bulanEND)) {
            $row = $db->query($sqlBulanPertama, [$biodataId, $yearPeriod])->getRow();
            return $row->month ?? null;
        }

        // 4️⃣ Cari bulan pertama setelah END
        $sqlStartKerja = "
        SELECT month FROM tr_tax
        WHERE biodata_id = ?
        AND year = ?
    ...
    ";
        ...
        // 5️⃣ Cek apakah ada jeda bulan
        if ($start_kerja && ($start_kerja - $bulanEND) > 1) {
            return $start_kerja;
        }

        // 6️⃣ Jika tidak ada jeda → kembali ke bulan pertama
        $row = $db->query($sqlBulanPertama, [$biodataId, $yearPeriod])->getRow();
        return $row->month ?? null;
    }
