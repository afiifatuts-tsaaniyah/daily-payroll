     ...
     public function exportSummaryPayrollMtb($yearPeriod, $monthPeriod)
    {
        ...
        foreach ($data as $row) {        
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('AH'.($rowIdx), '=X'.$rowIdx.'- (AA'.$rowIdx.' + AB'.$rowIdx.' + AD'.$rowIdx.' + AE'.$rowIdx.' + AF'.$rowIdx.' + AG'.$rowIdx.')')
                ->setCellValue('AI'.($rowIdx), $dueDateTotal)     
                ;     
        ...
        $spreadsheet->getActiveSheet()->setCellValue('AH'.($rowIdx+2), date('Y-m-d H:i:s'));
