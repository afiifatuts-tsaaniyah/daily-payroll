...
class Mtb_summary extends BaseController
{
     ...
     public function exportSummaryPayrollMtb($yearPeriod, $monthPeriod)
    {
        ...
        // Add some data
        $spreadsheet->getActiveSheet()->getStyle('A6:AK7')
            ->getFill()
        ...
        $spreadsheet->getActiveSheet()->getStyle("A6:AK7")->getFont()->setBold(true)->setSize(12);
        ...
        $spreadsheet->getActiveSheet()->getStyle("A6:AK7")->applyFromArray($allBorderStyle);
        $spreadsheet->getActiveSheet()->getStyle("A6:AK7")->applyFromArray($center);
        $spreadsheet->getActiveSheet()
            ->setCellValue('A6', 'NO')
            ...
            ->setCellValue('AG6', 'ADJUSTMENT / DEDUCTION')
            ->setCellValue('AH6', 'TOTAL')
            ->setCellValue('AI6', 'ROUNDED')
            ->setCellValue('AJ6', 'NET PAYMENT')
            ->setCellValue('AK6', 'DUE DATE');
        ...
        foreach ($data as $row) {        
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('N'.($rowIdx), $row['monthly'])
                ...
                ->setCellValue('W'.($rowIdx), $dataSum['unpaid'])       
                ->setCellValue('X'.($rowIdx), $dataSum['total_kotor'])
                ->setCellValue('Y'.($rowIdx), $golongan)
                ->setCellValue('Z'.($rowIdx), $tarif.'%')
                ->setCellValue('AA'.($rowIdx), $pajak)
                ->setCellValue('AB'.($rowIdx), $dataSum['jkkjkm'])
                ->setCellValue('AC'.($rowIdx), $dataSum['bpjs'])
                ->setCellValue('AD'.($rowIdx), $dataSum['emp_bpjs'])
                ->setCellValue('AE'.($rowIdx), $dataSum['emp_jht'])
                ->setCellValue('AF'.($rowIdx), $dataSum['emp_jp'])
                ->setCellValue('AG'.($rowIdx), $dataSum['adjustment'])
                ->setCellValue('AH'.($rowIdx), $total_bersih)
                ->setCellValue('AI'.($rowIdx), $t_rounded)
                ->setCellValue('AJ'.($rowIdx), $net_payment)     
                ->setCellValue('AK'.($rowIdx), $dueDateTotal)     
                ;     

            /* SET ROW COLOR */
            if($rowIdx % 2 == 1)
            {
                $spreadsheet->getActiveSheet()->getStyle('A'.$rowIdx.':AK'.$rowIdx)
                ->getFill()
                ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                ->getStartColor()
                ->setRGB('EAEBAF');             
        ...
        // exit();
        $spreadsheet->getActiveSheet()->setCellValue('AJ'.($rowIdx+2), date('Y-m-d H:i:s'));

        $spreadsheet->getActiveSheet()->getStyle("A6:AK".($rowIdx))->applyFromArray($allBorderStyle);
        // Rename worksheet
