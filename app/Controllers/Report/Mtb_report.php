...
class Mtb_report extends BaseController
{
    ...
    public function exportSummaryPayrollMartabeAgr($yearPeriod, $sm, $monthPeriod)
    {
        ...
        $spreadsheet->getActiveSheet()->getRowDimension(6)->setRowHeight(45);
        ...
        $spreadsheet->getActiveSheet()->mergeCells("A3:" . $lastColumn . "3");
        $spreadsheet->getActiveSheet()->getStyle("A2:" . $lastColumn . "2")->applyFromArray($center);
        $spreadsheet->getActiveSheet()->getStyle("A3:" . $lastColumn . "3")->applyFromArray($center);
        ...
        /* SET HEADER BG COLOR*/
        $spreadsheet->getActiveSheet()->getStyle("A6:" . $lastColumn . "6")
            ->getFill()
        ...
        /* START INVOICE TITLE */
        $spreadsheet->getActiveSheet()->getStyle("A6:" . $lastColumn . "6")->getFont()->setSize(12);
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('Q6', 'ADJUSTMENT IN')
            ->setCellValue('R6', 'ADJUSTMENT OUT')
            ->setCellValue('S6', 'JKK/JKM')
            ->setCellValue('T6', 'BPJS KES 5%')
            ->setCellValue('U6', 'JHT & JP')
            ->setCellValue('V6', 'PPH 21')
            ->setCellValue('W6', 'ADJ')
            ->setCellValue('X6', 'THR BY USER')
            ->setCellValue('Y6', 'TOTAL')
            ->setCellValue('Z6', 'ROUNDED')
            ->setCellValue('AA6', 'GAJI')
            ->setCellValue('AB6', 'KETERANGAN');
        ...
        foreach ($MultiSM as $key => $value) {
            ...
            foreach ($data as $row) {
                ...
                $spreadsheet->getActiveSheet()
                    ...
                    ->setCellValue('G' . $rowIdx, $row['gajipokok'])
                    ->setCellValue('H' . $rowIdx, $row['bs_prorate'])
                    ...
                    ->setCellValue('Q' . $rowIdx, $row['adjustment_in'])
                    ->setCellValue('R' . $rowIdx, $row['adjustment_out'])
                    ->setCellValue('S' . $rowIdx, $jkkjkm1)
                    ->setCellValue('T' . $rowIdx, $empbpjs1)
                    ->setCellValue('U' . $rowIdx, $jhtjp1)
                    ->setCellValue('V' . $rowIdx, - ($pph211))
                    ->setCellValue('W' . $rowIdx, $row['adjustment'])
                    ->setCellValue('X' . $rowIdx, $row['thr_by_user'])
                    ->setCellValue('Y' . $rowIdx, "=SUM(H" . $rowIdx . ":X" . $rowIdx . ")")
                    ->setCellValue('Z' . $rowIdx, $row['round'])
                    ->setCellValue('AA' . $rowIdx, $row['totalgaji'])
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('Y' . ($rowIdx + 1), "=SUM(Y8:Y" . $rowIdx . ")")
            ->setCellValue('Z' . ($rowIdx + 1), "=SUM(Z8:Z" . $rowIdx . ")")
            ->setCellValue('AA' . ($rowIdx + 1), "=SUM(AA8:AA" . $rowIdx . ")")
            ->setCellValue('AB' . ($rowIdx + 1), "=SUM(AB8:AB" . $rowIdx . ")")
        ;
        $spreadsheet->getActiveSheet()->getStyle("A" . ($rowIdx + 1) . ":" . $lastColumn . "" . ($rowIdx + 1))->getFont()->setBold(true)->setSize(12);
        ...
        $spreadsheet->getActiveSheet()->getStyle('E8:AA' . ($rowIdx + 1))->getNumberFormat()->setFormatCode('#,##0.00');
        $spreadsheet->getActiveSheet()->mergeCells("A" . ($rowIdx + 1) . ":F" . ($rowIdx + 1));
        $spreadsheet->getActiveSheet()->getStyle("A" . ($rowIdx + 1) . ":" . $lastColumn . "" . ($rowIdx + 1))->applyFromArray($outlineBorderStyle);
        $spreadsheet->getActiveSheet()->getStyle("A" . ($rowIdx + 1) . ":" . $lastColumn . "" . ($rowIdx + 1))->applyFromArray($allBorderStyle);
        $spreadsheet->getActiveSheet()->getStyle("A" . ($rowIdx + 1) . ":" . $lastColumn . "" . ($rowIdx + 1))->applyFromArray($center);


        $spreadsheet->getActiveSheet()->getStyle("A" . ($rowIdx + 1) . ":" . $lastColumn . "" . ($rowIdx + 1))
            ->getFill()
            ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
            ->getStartColor()
            ->setRGB('F2BE6B');
...
")
            ->setCellValue('B' . ($rowIdx + 18), "(  Andri Wijaya  )                                                      (                           )
")
