...
class Tax_calculation extends BaseController
{
    ...
    public function getMtbArList($tahun, $clientName)
    {
        ...
        if ($clientName == 'Agincourt_Martabe') {
    ...
    public function exportTaxCalculation($yearPeriod)
    {
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('W7', 'ADJUSTMENT IN')
            ->setCellValue('X7', 'ADJUSTMENT OUT')
        ...
        foreach ($data as $row) {
            ...
            $total_nonreguler_income =  $row['adjustment_in'] +  $row['adjustment_out'];
            ...
            if ($row['month_period'] == 11 or $row['check_final'] == 'END') {
                ...
                // dd($bruttoSetahuan.' '.$jpjht.' '.$tunjanganJabatan);
                ...
                $annualisation         = $total_income_ytd_idr - ($tunjangan_jabatan + $emp_jp_ytd + $emp_jht_ytd); // Penambahan tgl 17/12/2025 untuk end montly
                // dd($annualisation.' = '.$total_income_ytd_idr.' '.$tunjangan_jabatan.' '.$emp_jp_prev.' '.$emp_jht_prev);

                if ($annualisation >= $ptkpTotal) {
                    // if ($nettoSetahun >= $ptkpTotal) { // Perubahan tgl 17/12/2025 untuk end montly
                    $penghasilanPajak           = $annualisation - $ptkpTotal;
                    // $penghasilanPajak           = $nettoSetahun - $ptkpTotal; // Perubahan tgl 17/12/2025 untuk end montly
                ...
                $tax_due_income_per_annum = $pajakGajiSthn;

                $taxThisMonth   = $tax_due_income_per_annum - $taxDuePaidYTD;
            ...
            $gajiSudahDiBayar =  $row['thr_by_user'] + $row['adjustment'];
            $emp_bpjs       = $emp_jp;
            // Take Home Pay
            $takeHomePay = $total_income_this_month - ($taxThisMonth + $row['bpjs'] + $emp_bpjs + $row['jkkjkm'] + $emp_jp + $emp_jht) + $gajiSudahDiBayar;
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('L' . ($rowIdx), $row['basic_prorate']) // 1
                ->setCellValue('M' . ($rowIdx), $row['total_overtime']) // 2
                ->setCellValue('N' . ($rowIdx), $row['other_allowance']) // 3
                ->setCellValue('O' . ($rowIdx), $row['thr']) // 4
                ->setCellValue('P' . ($rowIdx), $row['night_shift']) // 5
                ->setCellValue('Q' . ($rowIdx), $row['day_shift']) // 6
                ->setCellValue('R' . ($rowIdx), '') // 7

                ->setCellValue('S' . ($rowIdx), $row['jkkjkm']) // 8
                ->setCellValue('T' . ($rowIdx), $row['bpjs']) // 9
                ->setCellValue('U' . ($rowIdx), $row['potongan_absensi']) // 10
                ->setCellValue('V' . ($rowIdx), $total_reguler_income)

                ->setCellValue('W' . ($rowIdx),  $row['adjustment_in'])
                ->setCellValue('X' . ($rowIdx),  $row['adjustment_out'])
                ...
                ->setCellValue('AG' . ($rowIdx), $emp_jp_ytd) //$emp_jp_ytd = $emp_jp+$emp_jp_prev;
                ...
                ->setCellValue('AN' . ($rowIdx), $penghasilanPajak)
        ...
        // FORMAT ANGKA
        $spreadsheet->getActiveSheet()
            ->getStyle("K{$startRow}:AP{$totalRow}")
            ->getNumberFormat()
            ->setFormatCode('#,##0.00');

        // FORMAT ANGKA
        $spreadsheet->getActiveSheet()
            ->getStyle("AS{$startRow}:BE{$totalRow}")
            ->getNumberFormat()
            ->setFormatCode('#,##0.00');
