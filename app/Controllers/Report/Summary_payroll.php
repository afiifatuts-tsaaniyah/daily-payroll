...
class Summary_payroll extends BaseController
{
    public function index()
    {
        ...
        $data['actView'] = 'Report/summary_payroll';
        return view('home', $data);
    }

    public function getDataList($clientName, $year, $month, $dataGroup)
    {
        $mSalarySlip = new SalarySlip();
        $results = $mSalarySlip->getSummaryPayrollData($clientName, $year, $month, $dataGroup);
        $myData = [];
        // var_dump($results);
        foreach ($results as $row) {
            ...
            $myData[] = [
                $row->slip_id,
                $row->full_name . ' - ' . $row->biodata_id,
                $row->client_name,
                $row->dept ?? $row->payroll_group,
                $row->position
            ];
        }
        return ResponseFormatter::success($myData, 'Summary Payroll retrieved successfully.', 200);
    }

    public function exportSumarryPayrollForPayroll($clientName, $yearPeriod, $monthPeriod, $dataGroup)
    {
        helper(['filesystem']);
        ...
        // Ambil data summary payroll
        $mSalarySlip = new SalarySlip();
        $dataSummaryPayroll = $mSalarySlip->getSummaryPayrollData(
            $clientName,
            $yearPeriod,
            $monthPeriod,
            $dataGroup
        ...
        // Pilih template export berdasarkan client
        if ($clientName == "Promincon_Indonesia") {
            $fileName = $this->exportSummaryPayrollPromincon(
                $clientName,
                $yearPeriod,
                $monthPeriod,
                $dataGroup,
            ...
            return "Client export method not found.";
        }

        // Path file sementara (CI4)
    ...
    public function exportSummaryPayrollPromincon($clientName, $yearPeriod, $monthPeriod, $dataGroup, $spreadsheet, $dataSummaryPayroll)
    {
    ...
    public function exportSumarryPayrollForAccounting($clientName, $yearPeriod, $monthPeriod, $dataGroup)
    {
        $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $exportController = new Summary_payroll_for_accounting();
        ...
        // Ambil data payroll
        $mSalarySlip = new SalarySlip();
        $dataSummaryPayroll = $mSalarySlip
            ->getListByBiorecIdSP($clientName, $yearPeriod, $monthPeriod, $dataGroup);
        // Eksekusi export sesuai client
        if (in_array($clientName, [
            'Promincon_Indonesia'
        ])) {
            $fileName = $exportController->exportSumarryPayrollForAccountingPromincon(
                $clientName,
                $yearPeriod,
                $monthPeriod,
                $dataGroup,
