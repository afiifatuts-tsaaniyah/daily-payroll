...
class Mtb_invoice extends BaseController
{
    ...
    public function exportinvoicePayrollMartabeAgr($yearPeriod, $monthPeriod)
    {
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('D'.($rowIdx+2), "=SUM(D12:D$firstRow)")
            ->setCellValue('E'.($rowIdx+2), "=SUM(E12:E$firstRow)")
            ->setCellValue('F'.($rowIdx+2), "=SUM(F12:F$firstRow)")
            ->setCellValue('G'.($rowIdx+2), "=SUM(G12:G$firstRow)")
            ->setCellValue('H'.($rowIdx+2), "=SUM(H12:H$firstRow)")
            ->setCellValue('I'.($rowIdx+2), "=SUM(I12:I$firstRow)")
            ->setCellValue('J'.($rowIdx+2), "=SUM(J12:J$firstRow)")
            ->setCellValue('K'.($rowIdx+2), "=SUM(K12:K$firstRow)")
            ->setCellValue('L'.($rowIdx+2), "=SUM(L12:L$firstRow)")
            ->setCellValue('M'.($rowIdx+2), "=SUM(M12:M$firstRow)")
            ->setCellValue('N'.($rowIdx+2), "=SUM(N12:N$firstRow)")
            ->setCellValue('O'.($rowIdx+2), "=SUM(O12:O$firstRow)")
            ->setCellValue('P'.($rowIdx+2), "=SUM(P12:P$firstRow)")
            ->setCellValue('Q'.($rowIdx+2), "=SUM(Q12:Q$firstRow)")
            ->setCellValue('R'.($rowIdx+2), "=SUM(R12:R$firstRow)")  
            ;
...
// exit();
    ...
    public function summaryTimesheet($yearPeriod, $monthPeriod, $dept)
    {
        ...
        // foreach ($dataId as $row) {
            ...
            return view('Report/summary_timesheet_new',$data);
        // }
    }
    ...
    public function exportNap($yearPeriod, $monthPeriod)
    {
        ...
        $angka = $subTotalInv + $managementFee + ($managementFee * 0.11);
        ...
        $tgl_new    = $yearPeriod."-".$monthPeriod."-15"; 
        $tgl1       = date('16 M Y', strtotime($tgl_new));
        $tgl2       = date('15 M Y', strtotime('+1 month', strtotime($tgl_new)));
