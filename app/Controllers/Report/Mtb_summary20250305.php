...
class Mtb_summary extends BaseController
{
     ...
     public function exportSummaryPayrollMtb($yearPeriod, $monthPeriod)
    {
        ...
        // Add some data
        $spreadsheet->getActiveSheet()->getStyle('A6:AJ7')
            ->getFill()
        ...
        $spreadsheet->getActiveSheet()->getStyle("A6:AJ7")->getFont()->setBold(true)->setSize(12);
        ...
        $spreadsheet->getActiveSheet()->getStyle("A6:AJ7")->applyFromArray($allBorderStyle);
        $spreadsheet->getActiveSheet()->getStyle("A6:AJ7")->applyFromArray($center);
        $spreadsheet->getActiveSheet()
            ->setCellValue('A6', 'NO')
        ...
        foreach ($data as $row) {        
            ...
            if($rowIdx % 2 == 1)
            {
                $spreadsheet->getActiveSheet()->getStyle('A'.$rowIdx.':AJ'.$rowIdx)
                ->getFill()
                ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                ->getStartColor()
                ->setRGB('EAEBAF');             
