...
class Mtb_summary extends BaseController
{
    ...
    public function exportSummaryPayrollMtb($yearPeriod, $monthPeriod)
    {
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('U6', 'ADJUSMENT IN')
            ->setCellValue('V6', 'ADJUSTMENT OUT')
            ->setCellValue('W6', 'JKK & JKM')
            ->setCellValue('X6', 'HEALTH BPJS')
            ->setCellValue('Y6', 'POTONGAN ABSENSI')
            ->setCellValue('Z6', 'GROSS')
            ->setCellValue('AA6', 'GOLONGAN')
            ->setCellValue('AB6', 'PERSENTASE')
            ->setCellValue('AC6', 'TAX TER THIS MONTH')
            ->setCellValue('AD6', 'JKK & JKM')
            // ->setCellValue('AC6', 'HEALTH BPJS')
            ->setCellValue('AE6', 'EMP HEALTH BPJS')
            ->setCellValue('AF6', 'EMP JHT')
            ->setCellValue('AG6', 'EMP JP')
            ->setCellValue('AH6', 'ADJUSTMENT / DEDUCTION')
            ->setCellValue('AI6', 'THR BY USER')
            ->setCellValue('AJ6', 'TOTAL')
            ->setCellValue('AK6', 'ROUNDED')
            ->setCellValue('AL6', 'NET PAYMENT')
            ->setCellValue('AM6', 'DUE DATE')
            ->setCellValue('AN6', 'STATUS EMP')
            ->setCellValue('AO6', 'NIK');
        ...
        foreach ($data as $row) {
            ...
            $total_bersih   = number_format($dataSum['total_kotor'] - ($pajak + $dataSum['jkkjkm'] + $dataSum['emp_bpjs'] + $dataSum['emp_jht'] + $dataSum['adjustment_in'] + $dataSum['adjustment_out'] + $dataSum['thr_by_user'] + $dataSum['emp_jp'] - ($dataSum['adjustment'])), 2, '.', '');
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('U' . ($rowIdx), $dataSum['adjustment_in'])
                ->setCellValue('V' . ($rowIdx), $dataSum['adjustment_out'])
                ->setCellValue('W' . ($rowIdx), $dataSum['jkkjkm'])
                ->setCellValue('X' . ($rowIdx), $dataSum['bpjs'])
                ->setCellValue('Y' . ($rowIdx), -$dataSum['potongan_absensi'])
                ->setCellValue('Z' . ($rowIdx), $dataSum['total_kotor'])
                ->setCellValue('AA' . ($rowIdx), $golongan . '' . $end . '')
                ->setCellValue('AB' . ($rowIdx), number_format($tarif, 2, ",", ".") . '%')
                ->setCellValue('AC' . ($rowIdx), -$pajak)
                ->setCellValue('AD' . ($rowIdx), -$dataSum['jkkjkm'])
                // ->setCellValue('AC'.($rowIdx), $dataSum['bpjs'])
                ->setCellValue('AE' . ($rowIdx), -$dataSum['emp_bpjs'])
                ->setCellValue('AF' . ($rowIdx), -$dataSum['emp_jht'])
                ->setCellValue('AG' . ($rowIdx), -$dataSum['emp_jp'])
                ->setCellValue('AH' . ($rowIdx), $dataSum['adjustment'])
                ->setCellValue('AI' . ($rowIdx), $dataSum['thr_by_user'])
                ->setCellValue('AJ' . ($rowIdx), $total_bersih)
                ->setCellValue('AK' . ($rowIdx), $t_rounded)
                ->setCellValue('AL' . ($rowIdx), $net_payment)
                ->setCellValue('AM' . ($rowIdx), $dueDateTotal)
                ->setCellValue('AN' . ($rowIdx), $row['emp_status'])
            ;
            $spreadsheet->getActiveSheet()->setCellValueExplicit('AO' . ($rowIdx), $row['id_card_no'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        ...
        // header di row 6
        // data mulai di row 7
        // $rowIdx sekarang = row data terakhir
        ...
        // Label TOTAL
        $spreadsheet->getActiveSheet()
            ->setCellValue("A{$totalRow}", 'TOTAL');

        // Merge sampai sebelum kolom angka
        $spreadsheet->getActiveSheet()
            ->mergeCells("A{$totalRow}:Y{$totalRow}");

        // Style TOTAL
        $spreadsheet->getActiveSheet()
            ->getStyle("A{$totalRow}:{$lastColumn}{$totalRow}")
            ->applyFromArray([
                'font' => ['bold' => true],
                'alignment' => [
        ...
        // FORMULA SUM
        $spreadsheet->getActiveSheet()
            ->setCellValue("Z{$totalRow}",  "=SUM(Z{$startRow}:Z{$endRow})")
            ->setCellValue("AC{$totalRow}", "=SUM(AC{$startRow}:AC{$endRow})")
            ->setCellValue("AD{$totalRow}", "=SUM(AD{$startRow}:AD{$endRow})")
            ->setCellValue("AE{$totalRow}", "=SUM(AE{$startRow}:AE{$endRow})")
            ->setCellValue("AF{$totalRow}", "=SUM(AF{$startRow}:AF{$endRow})")
            ->setCellValue("AG{$totalRow}", "=SUM(AG{$startRow}:AG{$endRow})")
            ->setCellValue("AJ{$totalRow}", "=SUM(AJ{$startRow}:AJ{$endRow})")
            ->setCellValue("AK{$totalRow}", "=SUM(AK{$startRow}:AK{$endRow})")
            ->setCellValue("AL{$totalRow}", "=SUM(AL{$startRow}:AL{$endRow})");

        // FORMAT ANGKA
        $spreadsheet->getActiveSheet()
            ->getStyle("N{$startRow}:Z{$totalRow}")
            ->getNumberFormat()
            ->setFormatCode('#,##0.00');

        // FORMAT ANGKA
        $spreadsheet->getActiveSheet()
            ->getStyle("AC{$startRow}:AL{$totalRow}")
            ->getNumberFormat()
            ->setFormatCode('#,##0.00');
        ...
        // exit();
        $spreadsheet->getActiveSheet()->setCellValue("" . $lastColumn . "" . ($rowIdx + 3), date('Y-m-d H:i:s'));
