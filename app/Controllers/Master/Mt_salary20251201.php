...
class Mt_salary extends BaseController
{
    ...
    public function ins()
    {
        ...
        // $jkkjkm = 1; 
        // }
        $data = $mtBiodata->getById($_POST['biodataId']);
        $userId = $_SESSION['uId'];
        ...
        $mtSalary->setClientName($_POST['client']);
        $mtSalary->setbpjs($bpjs);
        $mtSalary->setjht($jht);
        $mtSalary->setjp($jp);
        $mtSalary->setjkkjkm($jkkjkm);
        $mtSalary->setmonth($data['emp_status']);
    ...
    public function printSalary()
    {
        ...
        $data       = $this->db->query("SELECT a.*, b.bank_name FROM mt_salary a, mt_bank b WHERE a.`bank_id`=b.`bank_id`")->getResultArray();
        // dd($data);
        // echo $depart;
        // exit();
        ...
        $spreadsheet->getActiveSheet()->getStyle("A2:H2")->applyFromArray($center);
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('D4', 'NOMER REKENING')
            ->setCellValue('E4', 'NAMA REKENING')
            ->setCellValue('F4', 'MONTHLY')
            ->setCellValue('G4', 'DAILY')
            ->setCellValue('H4', 'STATUS PAYROLL')
        ;
        ...
        foreach ($data as $row) {
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('C' . $rowIdx, $row['bank_name'])
                ->setCellValue('D' . $rowIdx, $row['account_no'])
                ->setCellValue('E' . $rowIdx, $row['account_name'])
                ->setCellValue('F' . $rowIdx, $row['monthly'])
                ->setCellValue('G' . $rowIdx, $row['daily'])
                ->setCellValue('H' . $rowIdx, $row['status_payroll'])
            ;
