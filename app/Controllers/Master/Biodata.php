...
class Biodata extends BaseController
{
     ...
     public function ins()
     {
          ...
          $mtBiodata->setBpjsStatus($_POST['bpjsStatus']);
     ...
     public function printBiodata()
     {
        ...
        foreach ($data as $row) {
            ...
            $cell       = $spreadsheet->getActiveSheet()->getCell("M".$rowIdx);
            $spreadsheet->getActiveSheet()->setCellValueExplicit("M". $rowIdx, $cell->getValue(), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    // Kolom yang ingin diubah formatnya menjadi teks (misalnya kolom M)
    $columnToFormat = 'M';

    // Mendapatkan baris terakhir yang memiliki data di sheet
    $highestRow = $sheet->getHighestRow();

    // Loop melalui setiap baris di kolom dan mengatur formatnya menjadi teks
    for ($row = 1; $row <= $highestRow; $row++) {
        $cell = $sheet->getCell($columnToFormat . $row);
        $cell->getStyle()->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER);
    }
