      ...
      // exit();
        ...
        foreach ($data as $row) {
            ...
            for ($i=1; $i <= 31 ; $i++){
                ...
                if ($data == "SL" || $data == "AI" || $data == "AB" || $data == "MD") {
                    $hours = 0;
                }
               ...
               // echo $hours.'-'.$ot1.'</br>';
                ...
                if ($code == "DS" || $code == "NS" || $code == "AS" || $code == "SL" || $code == "MD") {
                    $absen = 1;
                } else {
                    $absen = 0;
                }
            ...
            if ($statusSalary == 'Monthly' || $statusSalary == 'month') {
                ...
                $bsProrate = ($monthly / $jmlHari) * ($jmlHari - $standByCount);
                $gaji = $bsProrate;
            }
    ...
    public function Excel($ts_id,$checkBpjs,$checkJHT,$checkJP,$checkJKKJKM,$spreadsheet, $noNewSheet)
    {
        ...
        // dd($row);
            ...
            /*echo $startDate.'</br>';*/
                ...
                $outVal = (int)$inVal + (double)$phHoursTmp +1;
                if ($outVal > 24) {
                    $outVal = $outVal - 24;
                } 
                if ((int)$inVal == 0) {
                ...
                if ($phCodeTmp == "SL" || $phCodeTmp == "AI" || $phCodeTmp == "AB" || $phCodeTmp == "MD" ) {
                    $phHoursTmp = 0;
                }
                ...
                if ($phCodeTmp == "DS" || $phCodeTmp == "NS" || $phCodeTmp == "AS" || $phCodeTmp == "SL" || $phCodeTmp == "MD" || $phCodeTmp == "AB" || $phCodeTmp == "AI") {
                    $absen = 1;
                } else {
                    $absen = 0;
                }
          ...
          // echo $phCodeTmp.$tunjanganMalam.'</br>'   ;
        ...
        $validasiDbId = $rpDb->getDataDbId($biodata_id, $dateProcessing, $ts_id);
        // echo $tanggal.'<br/>';
        // dd($validasiDbId);
        $rpDb->setIn($inVal);
        $rpDb->setOut($outVal);
        ...
        $rpDb->settotalot($totalOt);
        $rpDb->setot1($ot01Val);
        $rpDb->setot2($ot02Val);
        $rpDb->setot3($ot03Val);
        $rpDb->setot4($ot04Val);
        $rpDb->setnormalTime($normalTime);
        $rpDb->setdept($row['dept']);
        $rpDb->settunjangan($allTunjangan);
        $rpDb->settotal_ot_value($totalOtValue);
        $rpDb->settunjanganMalam($tunjanganMalam);
        $rpDb->settunjanganSiang($tunjanganSiang);
        if (!isset($validasiDbId)) {
            // echo 'insert <br/>';
            $headerId = $rpDb->generateId('db_id')['doc_id'];
            $rpDb->setDbId($headerId);
            $rpDb->ins();
        } else {
            // echo 'update '.$validasiDbId['db_id'].'<br/>';
            $rpDb->upd($validasiDbId['db_id']);
        }
            // echo $outVal ;
        }
        ...
        $spreadsheet->getActiveSheet()->setCellValue('C'.$totalTitle, '=SUM(F'.$totalTitle_sum.':J'.$totalTitle_sum.')');
      ...
      // $checkJKKJKM = $_POST['cbJKKM'];
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('P'.($taxCoordinate+1), $brutto)
            ->setCellValue('L'.($taxCoordinate+2), 'Iuran JHT JP(3%)')
            ->setCellValue('P'.($taxCoordinate+2), '-'.$JhtJp)
            ->setCellValue('L'.($taxCoordinate+3), 'Iuran JHT(3,7%)')
            ->setCellValue('P'.($taxCoordinate+3), '-'.$JHT2)            
            ->setCellValue('L'.($taxCoordinate+4), 'Tunjangan Jabatan')
            ->setCellValue('P'.($taxCoordinate+4), '-'.$TJabatan);

        $nettoCoordinate = 45;
        $netto = $brutto - $JhtJp - $JHT2 - $TJabatan - $Thr - $bonusdll;
        $netton = $brutto - $JhtJp - $JHT2 - $TJabatan;
        $nettoSetahun = $netto * 12;
        // echo $netto;
        // exit();

        $spreadsheet->getActiveSheet()->getStyle('L'.$nettoCoordinate.':P'.($nettoCoordinate))->applyFromArray($totalStyle);
        $spreadsheet->getActiveSheet()->getStyle('L'.$nettoCoordinate.':P'.($nettoCoordinate))->applyFromArray($topBorderStyle);
        $spreadsheet->getActiveSheet()
        ...
        $maxPajak2s = $maxPajak1s - 190000000;
        if ($maxPajak1s >= 190000000) {
            $taxVal2s = 28500000;
        } 
    
        if ($maxPajak1s > 190000000) {
            $taxVal3s = ($maxPajak1s - 190000000) * 0.25;
        }

        $maxPajak3s = $maxPajak2s - 250000000;
        if ($maxPajak2s >= 250000000) {
            $taxVal3s = 62500000;
        }       
        if ($maxPajak2s > 250000000) {
            $taxVal4s = ($maxPajak2s - 250000000) * 0.30;
        }
        $maxPajak4s = $maxPajak3s - 4500000000;
        if ($maxPajak3s >4500000000 ) {
            $taxVal4s = 1350000000;
        }
        if ($maxPajak4s > 4500000000) {
            $taxVal5s = ($maxPajak3s - 4500000000) * 0.35;
        }
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('L'.($tarifTaxCoordinate+6), 'Pajak Gaji Setahun')
            ->setCellValue('P'.($tarifTaxCoordinate+6), $pajakGajiSthn)
            ->setCellValue('L'.($tarifTaxCoordinate+7), 'Pajak Gaji Sebulan')
            ->setCellValue('P'.($tarifTaxCoordinate+7), $pajakPlusSebulan);
        ...
        $spreadsheet->getActiveSheet()
            ->setCellValue('L56', 'Tax Non Reguler')
            ->setCellValue('P56', $pajakNonReg)
            ->setCellValue('L57', 'Tax Pinalty')
            ->setCellValue('P57', $taxPinalty)
            ->setCellValue('L58', $text);
        $spreadsheet->getActiveSheet()->getStyle('L55:P55')->applyFromArray($totalStyle);
        $spreadsheet->getActiveSheet()->getStyle('L58')->applyFromArray($totalStyles);
        $spreadsheet->getActiveSheet()->getStyle('L55:P55')->applyFromArray($topBorderStyle);
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('P'.$outCoordinate, '-'.$taxTot)
