...
class Home extends BaseController
{
	...
	public function login()
	{
		if (isset($_POST['inputUser']) && isset($_POST['inputPassword'])) {
			...
			if ($rowCount > 0) {
				...
				$userClients = $this->getUserClients($tId);
				...
				$session->set('userClients', $userClients);
				$data['clients'] = $userClients;
	...
	public function getUserClients($userId)
	{
        ...
        SELECT 
            mt_client.client_id, 
            mt_client.client_name,
            mt_client.client_value
        FROM trn_user_client
        LEFT JOIN mt_client 
            ON mt_client.client_id = trn_user_client.client_id
        WHERE trn_user_client.user_id = ?
    ";

		$query = $db->query($sql, [$userId]); // binding parameter aman dari SQL Injection
