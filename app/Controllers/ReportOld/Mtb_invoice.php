...
class Mtb_invoice extends BaseController
{
    ...
    public function getPayrollListInvoiceDept($tahun,$bulan,$dept)
    {
        ...
        $data_proses = new M_tr_timesheet();
        $data = $data_proses->getDataInvoiceAllDept($tahun, $bulan, $dept);
        $myData = array();
        foreach ($data as $key => $row) 
        {
               $myData[] = array(
    ...
    public function exportinvoicePayrollMartabeAgr($yearPeriod, $monthPeriod)
    {
        ...
        foreach ($data as $row) {                      
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('G'.($rowIdx), $row['total_ot_value'])
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('D'.($rowIdx+2), "=SUM(D$rowIdx:D$firstRow)")
            ...
            ->setCellValue('H'.($rowIdx+2), "=SUM(H$rowIdx:H$firstRow)")
            ->setCellValue('I'.($rowIdx+2), "=SUM(I$rowIdx:I$firstRow)")
            ->setCellValue('J'.($rowIdx+2), "=SUM(J$rowIdx:J$firstRow)")
            ->setCellValue('K'.($rowIdx+2), "=SUM(K$rowIdx:K$firstRow)")
            ->setCellValue('L'.($rowIdx+2), "=SUM(L$rowIdx:L$firstRow)")
            ->setCellValue('M'.($rowIdx+2), "=SUM(M$rowIdx:M$firstRow)")
            ->setCellValue('N'.($rowIdx+2), "=SUM(N$rowIdx:N$firstRow)")
            ->setCellValue('O'.($rowIdx+2), "=SUM(O$rowIdx:O$firstRow)")
            ->setCellValue('P'.($rowIdx+2), "=SUM(P$rowIdx:P$firstRow)")
            ->setCellValue('Q'.($rowIdx+2), "=SUM(Q$rowIdx:Q$firstRow)")
            ->setCellValue('R'.($rowIdx+2), "=SUM(R$rowIdx:R$firstRow)")  
            ;
    ...
    public function exportinvoiceAllDept($yearPeriod, $monthPeriod, $dept)
    {
        ...
        foreach ($data as $row) {                      
            ...
            if ($statusPayroll == 'daily' || $statusPayroll == 'daily p') {
                ...
                $bpjsTk = $basicSalary * 20 * 0.0574;
                $bpjsKes = $basicSalary * 20 * 0.04; 
                $bpjsPen = $basicSalary * 20 * 0.02; 
            } else {
                $gajiPokok = $row['monthly'];
                $bpjsTk = $basicSalary * 0.0574;
                $bpjsKes = $basicSalary * 0.04;
                $bpjsPen = $basicSalary * 0.02; 
            }
            ...
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('F'.($rowIdx), $row['status_payroll'])
                ...
                ->setCellValue('M'.($rowIdx), $row['total_value_ot'])
        ...
        $spreadsheet->getActiveSheet()
            ...
            ->setCellValue('Q'.($rowIdx+2), "=SUM(Q$rowIdx:Q$firstRow)")
            ->setCellValue('R'.($rowIdx+2), "=SUM(R$rowIdx:R$firstRow)")
            ->setCellValue('S'.($rowIdx+2), "=SUM(S$rowIdx:S$firstRow)")
            ->setCellValue('T'.($rowIdx+2), "=SUM(T$rowIdx:T$firstRow)")
            ->setCellValue('U'.($rowIdx+2), "=SUM(U$rowIdx:U$firstRow)")  
            ->setCellValue('V'.($rowIdx+2), "=SUM(V$rowIdx:V$firstRow)")  
            ;
